...
u_first_row = u[:, 0]
vh_first_column = vh[0, :]
sqrt_s = np.sqrt(s[0])

filter_vertical = sqrt_s*u_first_row  # Creating vertical filter kernel
filter_horizontal = sqrt_s*vh_first_column  # Creating horizontal filter kernel

# Filtering the salt and peper picture
gflt_imns_filtered = conv1(imns, filter_vertical, mode='reflect', axis=-1)
gflt_imns_filtered = conv1(gflt_imns_filtered, filter_horizontal, mode='reflect', axis=-2)

# Filtering the noised picture
gflt_imng_filtered = conv1(imng, filter_vertical, mode='reflect', axis=-1)
gflt_imng_filtered = conv1(gflt_imng_filtered, filter_horizontal, mode='reflect', axis=-2)
