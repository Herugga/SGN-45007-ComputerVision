...
for i1 in range(npts1):
    for i2 in range(npts2):
       # upper = (np.sum((patches1[:,:,i1]-i2)*(patches2[:,:,i2])-i1))
        #down = np.sqrt((np.sum((patches1[:,:,i1]-i2)**2*np.sum(patches2[:,:,i2])-i1)**2))
        # ((np.sum((patches1[:,:,i1]-i2)*(patches2[:,:,i2])-i1)))/(np.sqrt(np.sum((patches1[:,:,i1]-i2)*(patches2[:,:,i2])-i1)))
        X = ((patches1[:,:,i1]))
        Y = ((patches2[:,:,i2]))
        mu_X = np.mean(X)
        mu_Y = np.mean(Y)
