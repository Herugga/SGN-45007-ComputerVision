        ...
        if not ret_val:
            ...
            if len(p0) <= 10:
                ...
                if len(faces) != 0:
                    ...
                    w = faces[0,2]
                    h = faces[0,3]
                    roi_gray = gray[x_up:(x_up+w),y_up:(y_up-h)]
                    ##-your-code-ends-here-##
                    ...
                    if p0.any():
                        ...
                        p0 = np.transpose(np.array([p0[:,0]+x_up,p0[:,1]+y_up]))
                    ##-your-code-ends-here-##
                ...
                np.array(isFound)
                if isFound.any():
                    indices = np.argwhere(isFound)[:,0]
                    goodpts = np.array(p1)[indices]
                ##-your-code-starts-here-##
                
                # Draw points using e.g. cv2.drawMarker
                ##-your-code-starts-here-##
                for i in range(goodpts.shape[0]):
                    cv2.drawMarker(img, (goodpts[i,0],goodpts[i,1]), color=(255,0,0))
                ##-your-code-ends-here-##
